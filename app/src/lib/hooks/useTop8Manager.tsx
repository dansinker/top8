...
export function useTop8Manager(viewDid?: string) {
    ...
    const searchFollows = async (query: string): Promise<Friend[]> => {
        ...
        try {
            ...
            const results = followsCache?.data?.reduce((acc: Friend[], follow) => {
                const followHandle = follow.handle.toLowerCase();
                const followDisplay = follow.displayName?.toLowerCase();
                if (followHandle.includes(searchQuery) || followDisplay?.includes(searchQuery)) {
                    acc.push({
                        ...follow,
                        uniqueId: `${follow.did}-${Date.now()}-${acc.length}`
                    });
                }
                return acc;
            }, []) || [];
        ...
        const initialize = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    const friendsWithIds = friendsList.map(
                        (friend: { subject: string }, index: number) => ({
                            ...friend,
