class PDSRecordManager {
    ...
    async putRecord(collection, rkey, record) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to save record: ${response.status} - ${errorBody}`);
            }
    ...
    async getRecord(collection, rkey) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to get record: ${response.status} - ${errorBody}`);
            }
    ...
    async listRecords(collection, limit = 100, cursor = null) {
        ...
        if (!Number.isInteger(limit) || limit < 1 || limit > 1000) {
            ...
            throw new Error('Invalid limit parameter - must be integer between 1-1000');
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to list records: ${response.status} - ${errorBody}`);
            }
    ...
    async deleteRecord(collection, rkey) {
        ...
        try {
            ...
            console.debug('[PDSRecordManager] Sending delete request with headers', {
                headerKeys: Object.keys(headers)
            });
            ...
            console.debug('[PDSRecordManager] Delete request payload', requestBody);
