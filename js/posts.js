...
class PostManager {
    ...
    formatPosts(feed, handle) {
        ...
        try {
            ...
            console.debug(`${logPrefix} Filtered ${feed.length} -> ${filteredPosts.length} posts`);
            ...
            return limitedPosts.map((item, index) => {
                const post = item.post;
                ...
                try {
                    const formattedPost = {
                        ...
                        author: {
                            name: this.sanitizeString(author.displayName) || author.handle || "Unknown",
                    ...
                    console.debug(`${logPrefix} Formatted post ${index + 1}/${limitedPosts.length}:`, formattedPost);
                    return formattedPost;
                } catch (formatError) {
                    console.error(`${logPrefix} Error formatting individual post:`, formatError);
                    return null;
                }
