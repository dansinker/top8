...
class AuthManager {
    ...
    async authenticate(username, password) {
        console.debug("[AuthManager] Starting authentication attempt", {
            username,
        });
        ...
        try {
            ...
            if (!authData?.accessJwt || !authData?.refreshJwt) {
                console.error("[AuthManager] Invalid auth response data", {
                    authData,
                });
    ...
    setTokens(accessToken, refreshToken) {
        ...
        if (!accessToken || typeof accessToken !== "string") {
            console.error("[AuthManager] Invalid access token provided", {
                accessToken,
            });
        ...
        if (!refreshToken || typeof refreshToken !== "string") {
            console.error("[AuthManager] Invalid refresh token provided", {
                refreshToken,
            });
        ...
        if (!jwtRegex.test(accessToken)) {
            console.error(
                "[AuthManager] Access token fails JWT format validation",
            );
        ...
        if (!jwtRegex.test(refreshToken)) {
            console.error(
                "[AuthManager] Refresh token fails JWT format validation",
            );
        ...
        try {
            ...
            this.did = payload.sub;

            console.debug("[AuthManager] Tokens and DID set successfully", {
    ...
    async refreshSession() {
        ...
        if (!this.refreshToken) {
            console.debug(
                "[AuthManager] No refresh token available, aborting refresh",
            );
            return false;
        }
        ...
        try {
            ...
            if (!refreshData?.accessJwt || !refreshData?.refreshJwt) {
                console.error("[AuthManager] Invalid refresh response data", {
                    refreshData,
                });
    ...
    getAuthHeaders(contentType = "application/json") {
        ...
        if (typeof contentType !== "string" || !contentType.trim()) {
            console.warn(
                "[AuthManager] Invalid content type provided, using default",
            );
    ...
    clearTokens() {
        ...
        try {
            ...
            if (this.accessToken !== null || this.refreshToken !== null) {
                console.error(
                    "[AuthManager] Failed to clear tokens completely",
                );
